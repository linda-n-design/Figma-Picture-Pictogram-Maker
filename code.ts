...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'create-pictogram') {
    ...
    frame.resize(
      cols * iconSize + (cols - 1) * horizontalSpacing,
      rows * iconSize + (rows - 1) * verticalSpacing
    );
...
async function createPersonIcon(size: number, isHighlighted: boolean, color: RGB): Promise<FrameNode> {
